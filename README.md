...
Vreate visualizations and/or tables which highlight news events containing coverage peaks for certain companies/special topics or other anomalies/patterns/cluster in the news article data.
Build a model in order to predict the likelihood of an article being read. Be mindful of and employ strategies accordingly when dealing with an unbalanced data set.
